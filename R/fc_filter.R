...
#' @param text_pattern Structure that will have the text in each of the boxes. It recognizes n, N and perc within brackets. For default it is "\{n\} (\{perc\}\%)". The label is always placed at the beginning, followed by a line break where the structure specified by `text_pattern` is placed.
...
#' @param text_pattern_exc Structure that will have the text in each of the boxes. It recognizes n, N and perc within brackets. For default it is "\{n\} (\{perc\}\%)". The label is always placed at the beginning, followed by a line break where the structure specified by `text_pattern_exc` is placed.
...
fc_filter <- function(object, filter = NULL, N = NULL, label = NULL, text_pattern = "{n} ({perc}%)", perc_total = FALSE, show_exc = FALSE, direction_exc = "right", label_exc = "Excluded", text_pattern_exc = "{n} ({perc}%)", sel_group = NULL, round_digits = 2, just = "center", text_color = "black", text_fs = 8, text_fface = 1, text_ffamily = NA, text_padding = 1, bg_fill = "white", border_color = "black", width = NA, height = NA, just_exc = "center", text_color_exc = "black", text_fs_exc = 6, text_fface_exc = 1, text_ffamily_exc = NA, text_padding_exc = 1, bg_fill_exc = "white", border_color_exc = "black", offset_exc = NULL, width_exc = NA, height_exc = NA) {
...
fc_filter.fc <- function(object, filter = NULL, N = NULL, label = NULL, text_pattern = "{n} ({perc}%)", perc_total = FALSE, show_exc = FALSE, direction_exc = "right", label_exc = "Excluded", text_pattern_exc = "{n} ({perc}%)", sel_group = NULL, round_digits = 2, just = "center", text_color = "black", text_fs = 8, text_fface = 1, text_ffamily = NA, text_padding = 1, bg_fill = "white", border_color = "black", width = NA, height = NA, just_exc = "center", text_color_exc = "black", text_fs_exc = 6, text_fface_exc = 1, text_ffamily_exc = NA, text_padding_exc = 1, bg_fill_exc = "white", border_color_exc = "black", offset_exc = NULL, width_exc = NA, height_exc = NA) {
  ...
  if (is.character(label) | is.expression(label)) {
    ...
    text_pattern_exp <- tryCatch(
      parse(text = text_pattern),
      error = \(e) {
        list(as.character(text_pattern))
    })
    new_fc <- new_fc |>
      dplyr::mutate(text = list(substitute(atop(x, y),
                                list(x = label[[1]],
                                     y = text_pattern_exp[[1]]))))
  ...
  if(show_exc) {
    ...
    if (is.character(label_exc) | is.expression(label_exc)) {
      text_pattern_exc <- text_pattern_exc |>
        as.character() |>
        stringr::str_glue(.envir = rlang::as_environment(new_fc2))
      text_pattern_exc_exp <- tryCatch(
        parse(text = text_pattern_exc),
        error = \(e) {
          list(as.character(text_pattern_exc))
      })

      new_fc2 <- new_fc2 |>
        dplyr::mutate(text = list(substitute(
          atop(x, y), list(x = label_exc[[1]], y = text_pattern_exc_exp[[1]])
        )))
